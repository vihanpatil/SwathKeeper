...
always describe exactly what's physically in the Gazebo world):
  ...
  1. sim/worlds/farmguard_field.sdf      -- the Gazebo world (field ground plane, orchard tree
                                             ...
                                             dynamic obstacles, the iris_with_gimbal vehicle).
...
def sdf_ground_plane(polygon_m) -> str:
...
def sdf_vehicle_include() -> str:
    # Same model URI + spawn pose as ardupilot_gz's iris_runway.sdf -- spawns at field polygon
    # origin (0,0), which is also the mission's home / first waypoint.
    return """
    <include>
...
def sdf_tree(obstacle: dict, defaults: dict) -> str:
...
def sdf_bird_actor(bird: dict) -> str:
...
def build_sdf(field_cfg, static_cfg, birds_cfg, obstacles) -> str:
    parts = [sdf_world_header(field_cfg)]
    parts.append(sdf_ground_plane(field_cfg["polygon_m"]))
    parts.append(sdf_vehicle_include())
    ...
    for obs in obstacles:
        parts.append(sdf_tree(obs, static_cfg["tree_defaults"]))
    ...
    for bird in birds_cfg["birds"]:
        parts.append(sdf_bird_actor(bird))
    parts.append("\n  </world>\n</sdf>\n")
    return "".join(parts)
...
def main():
    ...
    sdf_text = build_sdf(field_cfg, static_cfg, birds_cfg, obstacles)
    ...
    print(f"[gen_farm_world] well-formed XML confirmed; wrote world "
          f"({len(birds_cfg['birds'])} bird actors, {len(obstacles)} trees) -> {args.world_out}")
