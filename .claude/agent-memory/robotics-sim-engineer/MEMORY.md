...
- [Forward depth sensor (ADR-019)](forward_depth_sensor.md) — the second aperture: mount pose + the Gazebo +X generalization, gz depth_camera source facts, gate margins, exit contracts, the host-only synth-frame rehearsal.
- [gz teleport + park (set_pose/Physics)](gz_teleport_and_park.md) — set_pose applies ONLY via Physics (`data: true` = queued); park with zero gravity, never `<static>`; pgrep/zombie/scene-edge traps.
