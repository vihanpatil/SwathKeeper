...
| D | 🔜 Enable AP_DDS + lock interface names | `flight-software-engineer` | `tech-lead`, `devops-reliability-engineer` | `/ap/*` topics live; topic/frame names pinned in DECISIONS.md |
| E | Scenario scaffolding (skeletons that can't let a cell be silently skipped) | `qa-safety-reviewer` | — | Draft scenarios ready so Week 3 avoidance is testable from day 1 |
