...
> **FROZEN RECORD (archived 2026-08-25).** This was the ADR-007 gate runbook; all four gates closed
> green (Gate 0 on 2026-08-05, Gates 1-3 on 2026-08-18) and it has been a record, not a to-do, ever
> since — it moved out of `docs/runbooks/` so that shelf only holds things you actually run. Nothing
> in it was edited; it lived at `docs/runbooks/NDVI_VALIDATION.md` and a stub still stands there
> because `docs/DECISIONS.md` is append-only and links that path.
> **What supersedes it:** value gates alone are not enough — run `scripts/verify_mount_geometry.sh`
> after any mount or georef change (ADR-007 amendment 5: the mount faced the horizon through all
> four of these passes). To record a flight, use `docs/runbooks/FULL_PIPELINE_DEMO.md`; for current
> status, `docs/ROADMAP.md`.
> **Two things in here are still live** and are restated where they are needed: the three
> container-ephemeral `ros_gz_bridge` apt deps (also in `FULL_PIPELINE_DEMO.md` Shell 0) and the
> `drive_birds.py --once` bird-parking invocation.
