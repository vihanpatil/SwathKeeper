...
> **FROZEN RECORD (archived 2026-08-25).** This was never a procedure — it is a timeboxed plan plus
> an honest feasibility verdict on running a rendering Gazebo sim in hosted CI. The image-build half
> went green on 2026-08-18 (`sim-image.yml`); the headless-render smoke half has never run. It moved
> out of `docs/runbooks/` so that shelf only holds things you actually run. Nothing in it was edited;
> it lived at `docs/runbooks/SIM_CI.md` and a stub still stands there because `docs/DECISIONS.md` is
> append-only and links that path twice.
> **What supersedes it:** the verdict is now **ADR-008** in `docs/DECISIONS.md` (sim CI pulls a
> prebuilt image and stays `workflow_dispatch` until one run is green), and the deferral of its open
> items — "What needs the human" steps 1-4 — is recorded in `docs/ROADMAP.md`'s cut/deferred log
> (2026-08-25). What actually runs today is `.github/workflows/ci.yml` + `sim-image.yml`.
